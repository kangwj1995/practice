...
class Classifier(object):
    ...
    def build(self, x, reuse=None):
        ...
        with tf.variable_scope('model', reuse=reuse):
            conv1 = tf.nn.max_pool(tf.nn.relu(tf.nn.conv2d(x, tf.Variable(tf.random_normal(shape=[3, 3, 1, 32])), strides=[1, 1, 1, 1], padding='SAME')+tf.Variable(tf.zeros([32]))), ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            conv2 = tf.nn.max_pool(tf.nn.relu(tf.nn.conv2d(conv1, tf.Variable(tf.random_normal(shape=[3, 3, 32, 64])), strides=[1, 1, 1, 1], padding='SAME') + tf.Variable(tf.zeros([64]))), ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            fc1 = tf.layers.dense(tf.contrib.layers.flatten(conv2), units=512, activation=tf.nn.relu)
    ...
    def train(self):
        ...
        # load data
        tr_img, tr_lab = read_tfrecord(self.args.train_datadir+'/train.tfrecord', self.args.batch, self.args.epoch)
        val_img, val_lab = read_tfrecord(self.args.val_datadir+'/val.tfrecord', self.args.batch, self.args.epoch)
    ...
    def test(self):
        # load data
        ts_img, ts_lab = read_tfrecord(self.args.test_datadir+'/test.tfrecord', self.args.batch, None)
